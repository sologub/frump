...
impl TemplateManager {
    ...
    pub fn load(&self) -> Result<Vec<TaskTemplate>> {
        ...
        let content =
            fs::read_to_string(&self.templates_file).context("Failed to read templates file")?;
    ...
    pub fn save(&self, templates: &[TaskTemplate]) -> Result<()> {
        ...
        fs::write(&self.templates_file, content).context("Failed to write templates file")?;
...
mod tests {
    ...
    fn test_template_placeholders() {
        ...
        replacements.insert(
            ...
            "crashes on empty input".to_string(),
        );

        let manager = TemplateManager::new();
        let _task = manager
            .instantiate("bug", TaskId::new(1).unwrap(), &replacements)
            .ok();
